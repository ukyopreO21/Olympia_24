...
const roundName = ["Start", "Obstacle", "Acceleration", "Finish", "Sub-Finish"];
export const vietnameseRoundName = ["KHỞI ĐỘNG", "VƯỢT CHƯỚNG NGẠI VẬT", "TĂNG TỐC", "VỀ ĐÍCH"];
...
socket.on("_resetStatus", (roundID) => {
    if (roundID == 2) {
        const allChar = document.querySelectorAll(".OBS_char");
        allChar.forEach((char) => {
            char.textContent = "";
            char.classList.remove("OBS_right-row");
            char.classList.remove("OBS_wrong-row");
            char.classList.remove("OBS_chosen");
        });
    } else if (roundID == 5) {
    } else if (roundID == 6) {
    }
...
socket.on("_playIntro", (roundID) => {
    ...
    intro.onended = () => {
        document.getElementById("round-name").textContent = vietnameseRoundName[roundID - 1];
    ...
    if (roundID == "1") {
        ...
        document.getElementById("acceleration-interface").style.visibility = "hidden";
        ...
        document.getElementById("start-interface").style.visibility = "hidden";
        document.getElementById("obstacle-interface").style.visibility = "hidden";
        document.getElementById("acceleration-interface").style.visibility = "visible";
        document.getElementById("finish-interface").style.visibility = "hidden";
        document.getElementById("sub-finish-interface").style.visibility = "hidden";
    } else if (roundID == "4") {
