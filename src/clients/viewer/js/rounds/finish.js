...
export const handle = () => {
    ...
    main.socket.on("_FIN_chooseQuestion", (data) => {
        ...
        if (main.getMediaType(data.questionData.mediaUrl) == "image") document.getElementById("FIN_image").innerHTML += "<img src='" + data.questionData.mediaUrl + "'>";
        for (let i = 1; i <= 3; i++) {
    ...
    main.socket.on("_FIN_5s", () => {
        const bg = document.getElementById("FIN_5s-background");
        bg.classList.remove("FIN_change-background");
        void bg.offsetWidth;
        bg.classList.add("FIN_change-background");
        for (let i = 1; i <= 4; i++) {
            document.getElementById("FIN_point-" + i).classList.remove("FIN_granted");
        }
...
const roundInterface = (visibility) => {
    ...
    document.getElementById("FIN_image").style.visibility = visibility;
