...
function updateData(data) {
	...
	XlsxPopulate.fromFileAsync("./Database/Database" + dbNumber + ".xlsx").then(
		(workbook) => {
			let sheetNames = [
				"Start",
				"Obstacle",
					...
					for (let p = 0; p < 5; p++) {
						if (p < 4) {
							for (let q = 0; q < 6; q++) {
								sheet
									.cell("C" + (2 + 6 * p + q))
									.value(data.data[i][p][q].subject);
								sheet
									.cell("D" + (2 + 6 * p + q))
									.value(data.data[i][p][q].question);
								sheet
									.cell("E" + (2 + 6 * p + q))
									.value(data.data[i][p][q].answer);
								sheet
									.cell("G" + (2 + 6 * p + q))
									.value(data.data[i][p][q].media);
							}
						} else {
							for (let q = 0; q < 12; q++) {
								sheet
		...
		for (let i = 0; i < 5; i++) {
			...
			if (obstacleDb[i].rowLength == null) {
				if (obstacleDb[i].answer == null) obstacleDb[i].rowLength = 0;
					...
					obstacleDb[i].rowLength = obstacleDb[i].answer.replace(
						/\s/g,
						""
					).length;
			}
	...
	if (!isReadBase64) {
		...
		for (let i = 0; i < 4; i++) {
			...
			if (accelerationDb[i].type == "Video") {
				...
				try {
					const base64String =
						"data:video/mp4;base64," + (await ACC_base64Video(videoSource));
					accelerationDb[i].source = base64String;
				} catch (error) {
					console.error(
						"Lỗi: Không thể mã hoá base64 video tăng tốc số " + (i + 1)
					);
				}
			} else {
				accelerationDb[i].source = Database[i + 1][5];
...
async function setData(dbNumber) {
	let matchData = await readDatabase(
		"./Database/Database" + dbNumber + ".xlsx",
		1
	);
...
function FIN_getQuestion(i) {
	if (FIN_questionPos[i - 1].point == 20)
		return FIN_QnA[FIN_currentPlayer - 1][FIN_questionPos[i - 1].pos];
...
io.on("connection", function (socket) {
	...
	socket.on("playerEnterRoom", function (playerNumber) {
		if (!validSlot[playerNumber - 1]) {
			if (userID[playerNumber - 1] == "") {
				socket.emit("firstLogIn", true);
			} else socket.emit("firstLogIn", userID[playerNumber - 1]);
			console.log("Player " + playerNumber + " entered the room.");
			clearInterval(disconnectTime);
			socket.emit("sendPlayersData", { playerName, playerPoint, isReady });
		} else socket.emit("_playerEnterRoom", false);
	});
	...
	socket.on("signOut", function (playerNumber) {
		let count = 0;
		disconnectTime = setInterval(function () {
			if (count == 5) {
	...
	socket.on("sendChat", function (dataUser) {
		...
		if (dataUser.playerNumber != 0)
			username = playerName[dataUser.playerNumber - 1] + dataUser.username;
	...
	socket.on("_currentUI", function (UIData) {
		io.emit("_playerEnterRoom", UIData);
	});
		...
		if (playerData.sourceNum == ACC_QnA[ACC_currentQuestion - 1].source.length)
			console.log(
				"Thí sinh " +
					...
					" đã tải thành công video tăng tốc " +
					ACC_currentQuestion
			);
		else
			console.log(
