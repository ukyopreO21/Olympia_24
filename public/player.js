...
document.getElementById("yourStartPosition").textContent =
	"Vị trí xuất phát: " + playerNumber;
...
socket.on("_sendReady", function (data) {
	if (data.ready)
		document.getElementById(
			"waitingRoom_player" + data.playerNumber
		).innerHTML += '<img class="ready" src="./Others/Ready.png"></img>';
	else {
		let ready = document.querySelector(
			"#waitingRoom_player" + data.playerNumber + " .ready"
		);
...
socket.on("firstLogIn", function (data) {
	if (typeof data != "boolean" && data != sessionStorage.getItem("userID")) {
		...
		socket.emit("saveUserID", { playerNumber, userID });
	}
});

socket.on("_playerEnterRoom", function (legit) {
	if (!legit) {
		signOutReason = "notAcceptable";
		window.location.replace("http://" + window.location.host);
	}
});

socket.on("sendPlayersData", function (data) {
	document.getElementById("waitingRoom_player" + playerNumber).style.color =
		"orange";
	for (let i = 1; i <= 4; i++) {
		//phòng chờ
		document.getElementById("waitingRoom_name" + i).textContent =
			data.playerName[i - 1];
		document.getElementById("waitingRoom_point" + i).textContent =
			data.playerPoint[i - 1];
		if (data.isReady[i - 1])
			document.getElementById("waitingRoom_player" + i).innerHTML +=
				'<img class="ready" src="./Others/Ready.png"></img>';
		//trong phòng thi
...
socket.on("_playerEnterRoom", function (UIData) {
	...
	if (UIData.isChatBan == false)
		document
			.getElementById("message-input")
			.addEventListener("keypress", sendText);
	else
		document
			.getElementById("message-input")
			.removeEventListener("keypress", sendText);
	document.getElementById("roundName").innerHTML = roundName[UIData.roundID];
});
...
socket.on("_resetStatus", function (roundID) {
	if (roundID == 2) {
		for (let i = 0; i < 4; i++)
			document.getElementById("OBS_Row" + (i + 1)).style.color = "white";
	}
});
...
function ContestUI() {
	...
	for (let i = 0; i < childElements.length; i++) {
		if (
			!Obstacle.contains(childElements[i]) &&
			!Finish.contains(childElements[i])
		)
			childElements[i].style.visibility = "visible";
	}
}
...
socket.on("_sendAdminData", function (adminData) {
	for (i = 1; i <= 4; i++) {
		document.getElementById("Name" + i).textContent =
			adminData.currentPlayerName[i - 1];
		document.getElementById("Point" + i).textContent =
			adminData.currentPlayerPoint[i - 1];
		document.getElementById("waitingRoom_point" + i).textContent =
			adminData.currentPlayerPoint[i - 1];
		document.getElementById("waitingRoom_name" + i).textContent =
			adminData.currentPlayerName[i - 1];
...
socket.on("_startRound", function () {
	...
	if (currentRoundID == "1") {
		parent.innerHTML +=
			'<div id="STR_Player"></div><div id="STR_Progress"></div><div id="STR_Subject"></div>';
		...
		parent.innerHTML +=
			'<div id="FIN_Star"><img></div><div id="FIN_Player"></div><div id="FIN_Pack"></div><div id="FIN_Question"></div>';
		document.getElementById("sendSignal").textContent = "GIÀNH QUYỀN TRẢ LỜI";
...
function countDown(startTime, offGranted) {
	...
	if (currentRoundID == 2)
		document
			...
			.addEventListener("keypress", sendAnswer);
	...
	downloadTimer = setInterval(function () {
		...
		if (timeLeft <= 0) {
			...
			if (currentRoundID == 2)
				document
					.getElementById("answerInput")
					.removeEventListener("keypress", sendAnswer);
		}
		timeLeft -= 1;
	}, 1000);
}
...
socket.on("_OBS_showRows", function (data) {
	...
	document.getElementById("OBS_Key").textContent =
		"CHƯỚNG NGẠI VẬT CÓ " + charNumber.length + " KÍ TỰ";
	ObstacleUI();
...
socket.on("_OBS_getRowsLength", function (data) {
	for (let i = 0; i < 4; i++)
		document.getElementById("OBS_Row" + (i + 1)).textContent =
			"Hàng ngang " + (i + 1) + " (" + data[i].rowLength + " kí tự)";
});
...
socket.on("_OBS_chooseRow", function (data) {
	...
	resetAnswerZone();
	if (data.rowIth == 5)
		document.getElementById("OBS_ACC_SFI_Status").textContent = "Ô trung tâm";
	else {
		document.getElementById("OBS_Row" + data.rowIth).style.color = "orange";
		document.getElementById("OBS_ACC_SFI_Status").textContent =
			"Hàng ngang " + data.rowIth;
	}
});
...
socket.on("_OBS_serverObsSignal", function (signalData) {
	...
	if (playerNumber == Number(signalData.signalDataFromAdmin.numberOfPlayer))
		document
			.getElementById("answerInput")
			.removeEventListener("keypress", sendAnswer);
	...
	).style.left = 25.25 * (Number(signalData.OBS_numberOfObsSignal) - 1) + "%";
});
...
socket.on("_OBS_showRowAnswer", function (rowAnswerData) {
	...
	for (let i = 1; i <= 4; i++) {
		document.getElementById("answerName" + i).textContent =
			rowAnswerData.name[i - 1];
		document.getElementById("answerText" + i).textContent =
			rowAnswerData.answer[i - 1];
	}
});
...
socket.on("_OBS_playRightRow", function (data) {
	...
	if (data.currentRow != 5) {
		document.getElementById("OBS_Row" + data.currentRow).textContent =
...
function OBS_showObstacle(obsData) {
	...
	document.getElementById("OBS_Key").textContent =
		"CHƯỚNG NGẠI VẬT: " + obsData.CNV;
	for (i = 0; i < 4; i++) {
		document.getElementById("OBS_Row" + (i + 1)).textContent =
			obsData.OBS_QnA[i].answer;
...
socket.on("_STR_choosePlayer", function (ithStart) {
	...
	if (STR_currentPlayer != 5)
		document.getElementById("STR_Player").innerHTML =
			"<font color='orange'>Lượt:</font>" +
			"&nbsp;" +
			allPlayerName[STR_currentPlayer - 1];
	else
		document.getElementById("STR_Player").innerHTML =
			"<font color='orange'>Lượt:</font>&nbsp" + "Chung";
});
...
socket.on("_STR_startPlayerTurn", function () {
	...
	if (STR_currentPlayer != 5)
		document.getElementById("STR_Progress").innerHTML =
			"<font color='orange'>Câu:</font>" + "&nbsp;" + "0/6";
	else
		document.getElementById("STR_Progress").innerHTML =
			"<font color='orange'>Câu:</font>" + "&nbsp;" + "0/12";
});
...
function STR_printNextQuestion(question) {
	...
	document.getElementById("STR_Subject").textContent =
		STR_ithQuestion + ". " + question.subject;
}

function STR_printPassStatus() {
	if (STR_currentPlayer != 5)
		document.getElementById("STR_Progress").innerHTML =
			"<font color='orange'>Câu:</font>" + "&nbsp;" + STR_ithQuestion + "/6";
	else
		document.getElementById("STR_Progress").innerHTML =
			"<font color='orange'>Câu:</font>" + "&nbsp;" + STR_ithQuestion + "/12";
}
...
socket.on("_STR_getNextQuestion", function (questionData) {
	if (STR_signalPlayer)
		document
			.getElementById("currentPoint" + STR_signalPlayer)
			.classList.remove("FIN_Granted");
	STR_signalPlayer = 0;
	STR_ithQuestion++;
...
socket.on("_STR_finishTurn", function () {
	...
	document.getElementById("sendSignal").onclick = "";
	for (let i = 1; i <= 4; i++)
		document.getElementById("currentPoint" + i).classList.remove("FIN_Granted");
	document.getElementById("questionText").textContent = "";
...
socket.on("_ACC_openQuestion", function (ACC_questionData) {
	...
	if (ACC_questionData.type == "Video") {
		...
		document.getElementById("Media").innerHTML +=
			"<img src='" + ACC_questionData.source + "'>";
	}
	resetAnswerZone();
});
...
socket.on("_ACC_sentAnswer", function (serverData) {
	if (playerNumber == serverData.answerData.playerNumber) {
		document.getElementById("answerInput").value = "";
		document.getElementById("saveAnswerText").textContent =
			serverData.answerData.playerAnswer + " (" + serverData.costTime + ")";
	}
});
...
function ACC_timing(time) {
	...
	document
		.getElementById("answerInput")
		.addEventListener("keypress", ACC_keypressHandler);
	ACC_mainTime.pause();
...
socket.on("_ACC_showAnswer", function (answerData) {
	document
		.getElementById("answerInput")
		.removeEventListener("keypress", ACC_keypressHandler);
	...
	for (i = 1; i <= 4; i++) {
		...
		document.getElementById("answerText" + i).innerHTML =
			answerData[i - 1].answer + "<br>" + "[" + time + "]";
	}
});
...
socket.on("_FIN_choosePlayer", function (player) {
	...
	document.getElementById("FIN_Player").innerHTML =
		"<font color='orange'>Lượt:</font>" +
		"&nbsp;" +
		allPlayerName[FIN_currentPlayer - 1];
});
...
socket.on("_FIN_Choose", function (chooseData) {
	...
	let list = document.querySelectorAll(
		".FIN_Question" + chooseData.ithQuestion
	);
...
socket.on("_FIN_packChosen", function (list) {
	...
	for (let i = 0; i < 3; i++) {
		document
			.querySelector("#FIN_Pack" + list[i] + " .FIN_Question" + (i + 1))
			.classList.add("FIN_Chosen");
...
socket.on("_FIN_chooseQuestion", function (question) {
	...
	document.getElementById("FIN_Question").textContent =
		"Câu " +
		question.ithQuestion +
		" - " +
		question.questionData.point +
		" điểm";
	document.getElementById("questionText").textContent =
...
socket.on("_SFI_startRound", function (playerList) {
	...
	for (let i = 0; i < playerList.length; i++) {
		...
		document.getElementById("answerName" + (i + 1)).textContent =
			allPlayerName[playerList[i] - 1];
	}
	document.getElementById("customStatus").innerHTML +=
		'<div id="OBS_ACC_SFI_Status"></div>';
});

socket.on("_SFI_openQuestion", function (serverData) {
	SFI_Eliminated = false;
	document.getElementById("OBS_ACC_SFI_Status").textContent =
		"Câu hỏi số " + serverData.openedCount;
	document.getElementById("questionText").textContent =
...
function SFI_Timing(startTime, timeLeft) {
	...
	downloadTimer = setInterval(function () {
		...
		document.getElementById("timeLeft").textContent =
			Math.floor(Number(timeLeft - (currentTime - startTime))) + 1;
