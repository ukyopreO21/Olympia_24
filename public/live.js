...
var OBS_showRowAnswerAudio = new Audio(
	"./Obstacle/Sounds/VCNVMoDapAnThiSinh.mp3"
);
...
var ACC_showAnswersAudio = new Audio(
	"./Acceleration/Sounds/TTHienDapAnThiSinh.mp3"
);
...
var FIN_showQuestionPackAudio = new Audio(
	"./Finish/Sounds/VDHienCacGoiCauHoi.mp3"
);
...
//CÂU HỎI PHỤ
var SFI_mainTime = new Audio(
	"./Obstacle/Sounds/VCNV_15GiayCuoi_CHP_15Giay.mp3"
);
...
socket.on("_sendAdminData", function (adminData) {
	...
	document.getElementById("FIN_currentPoint").textContent =
		allPlayerPoint[Number(FIN_currentPlayer) - 1];
});
...
socket.on("_OBS_showRows", function (OBS_Data) {
	...
	// socket.emit("getRowsLength");
	// for (let i = 1; i <= 5; i++)
	// 	document.getElementById("OBS_Q" + i).style.opacity = 1;
});

socket.on("_OBS_getRowsLength", function (data) {
	// for (let i = 0; i < 4; i++) {
	// 	for (
	// 		let j = data[i].startPos;
	// 		j < data[i].startPos + data[i].rowLength;
	// 		j++
	// 	) {
...
socket.on("_OBS_chooseRow", function (data) {
	...
	// var elements = document.getElementsByClassName("OBS_Char");
	...
	// 		j <
	// 		data.OBS_QnA[Number(data.rowIth - 1)].startPos +
	// 			data.OBS_QnA[Number(data.rowIth - 1)].rowLength;
	// 		j++
	// 	) {
	// 		setTimeout(function () {
...
socket.on("_OBS_showRowQuestion", function (questionIth) {
	...
	document
		.getElementById("OBS_questionZone")
		.classList.remove("OBS_moveQuestionBox");
	...
	document
		.getElementById("OBS_questionZone")
		.classList.add("OBS_moveQuestionBox");
...
socket.on("_OBS_serverObsSignal", function (signalData) {
	...
	document.getElementById(
		"OBS_Signal" + signalData.OBS_numberOfObsSignal
	).style.left =
		"calc(" + (Number(signalData.OBS_numberOfObsSignal) - 1) * 21 + "%)";
	document.getElementById(
		"OBS_Signal" + signalData.OBS_numberOfObsSignal
	).textContent =
		signalData.OBS_numberOfObsSignal +
...
socket.on("_OBS_showRowAnswer", function (rowAnswerData) {
	...
	// 	for (let i = 0; i < 4; i++) {
	// 		document.getElementById("name" + (i + 1)).innerHTML =
	// 			rowAnswerData.name[i];
	// 		document.getElementById("answer" + (i + 1)).innerHTML =
	// 			rowAnswerData.answer[i];
	// 	}
	// }, 2000);
});

socket.on("_OBS_backScreen", function () {
	// isOpeningRowAnswers = false;
...
socket.on("_OBS_wrongRow", function (numberOfWrongPlayer) {
	// var nameSelector = "name" + numberOfWrongPlayer;
	// var answerSelector = "answer" + numberOfWrongPlayer;
	// document.querySelector("." + nameSelector).style.opacity = "0.5";
	// document.querySelector("." + answerSelector).style.opacity = "0.5";
});

socket.on("_playRightRow", function (data) {
	// isOpeningQuestionBox = false;
	...
	// for (
	// 	let j = data.questionData.startPos;
	// 	j < data.questionData.startPos + data.questionData.rowLength;
	// 	j++
	// ) {
	// 	document.getElementById(
	// 		"OBS_Char" + data.currentRow + "." + j
	// 	).textContent = currentRowAnswer[j - data.questionData.startPos];
	// 	document.getElementById(
	// 		"OBS_Char" + data.currentRow + "." + j
...
function OBS_showObstacle(obsData) {
	...
	// 	for (
	// 		let j = obsData.OBS_QnA[i - 1].startPos;
	// 		j < obsData.OBS_QnA[i - 1].startPos + obsData.OBS_QnA[i - 1].rowLength;
	// 		j++
	// 	) {
	// 		let currentRowAnswer = String(obsData.OBS_QnA[i - 1].answer).replace(
	// 			/\s+/g,
	// 			""
	// 		);
	// 		document.getElementById("OBS_Char" + i + "." + j).textContent =
	// 			currentRowAnswer[j - obsData.OBS_QnA[i - 1].startPos];
	// 		document.getElementById("OBS_Char" + i + "." + j).style.backgroundImage =
	// 			"linear-gradient(to right bottom, #2b8ba6, #0083b0, #007ab9, #006ec0, #0060c1)";
	// 	}
	// }
}
...
socket.on("_OBS_rightObs", function (obsData) {
	...
	// showObstacle(obsData);
});

socket.on("_OBS_keepRightObs", function (value) {
	// document.getElementById("OBS_Signal" + value).style.opacity = "1";
	// for (let i = 1; i <= 4; i++) {
	// 	if (i != Number(value) && document.getElementById("OBS_Signal" + i))
	// 		document.getElementById("OBS_Signal" + i).style.opacity = "0.5";
	// }
});
...
socket.on("_STR_choosePlayer", function (ithStart) {
	...
	if (STR_currentPlayer != 5)
		document
			.getElementById("STR_Player" + STR_currentPlayer)
			.classList.add("STR_Granted");
});
...
function STR_printPlayerData() {
	for (let i = 0; i < 4; i++) {
		document.getElementById("STR_Name" + (i + 1)).textContent =
			i + 1 + ". " + allPlayerName[i];
		document.getElementById("STR_Point" + (i + 1)).textContent =
			allPlayerPoint[i];
	}
}
...
socket.on("_STR_openQuestionBoard", function () {
	...
	statusShelf.classList.remove("STR_moveStatusShelf");
	statusShelf.offsetWidth;
	statusShelf.classList.add("STR_moveStatusShelf");
});
...
function STR_printPassStatus() {
	if (STR_currentPlayer != 5)
		document.getElementById("STR_Status").textContent =
			"Câu " + STR_ithQuestion + "/6";
	else
		document.getElementById("STR_Status").textContent =
			"Câu " + STR_ithQuestion + "/12";
}
...
socket.on("_ACC_chooseQuestion", function () {
	...
	// //chạy animation
	// document
	// 	.getElementById("ACC_questionBar")
	// 	.classList.remove("ACC_flipDownQuestion");
	...
	// document
	// 	.getElementById("ACC_questionBar")
	// 	.classList.add("ACC_flipDownQuestion");
...
function ACC_removeAllTimingAnimation() {
	for (let i = 10; i <= 40; i += 10) {
		...
		document
			.getElementById("ACC_Circle")
			.classList.remove("ACC_moveCircle" + i);
	}
}
...
socket.on("_ACC_startTiming", function (timeData) {
	...
	// document
	// 	.getElementById("ACC_Circle")
	// 	.classList.add("ACC_moveCircle" + timeData.ithQuestion * 10);
	// document
	// 	.getElementById("ACC_Line")
	// 	.classList.add("ACC_moveLine" + timeData.ithQuestion * 10);
});
...
socket.on("_ACC_showAnswer", function (answerData) {
	...
	// for (let i = 4; i >= 1; i--) {
	// 	document
	// 		.getElementById("ACC_Player" + i)
	// 		.classList.remove("ACC_playerMove");
...
socket.on("_FIN_choosePlayer", function (player) {
	...
	for (let i = 1; i <= 4; i++) {
		document
			.getElementById("FIN_Point" + i)
			.classList.remove("FIN_differentPoint");
...
socket.on("_FIN_Choose", function (chooseData) {
	...
	for (let i = 20; i <= 30; i += 10) {
		document.getElementById(
			"FIN_" + chooseData.ithQuestion + "." + i
		).innerHTML = "";
	}
	var addCheck = document.getElementById(
		"FIN_" + chooseData.ithQuestion + "." + chooseData.ithPoint
	);
	addCheck.innerHTML +=
		'<img src="./Others/Check.png" width="100%" height="100%" class="FIN_Checked"></img>';
});
...
socket.on("_FIN_showQuestionPack", function () {
	...
	for (let i = 2; i <= 3; i++) {
		for (let j = 1; j <= 3; j++) {
			...
			document
				...
				.classList.remove("FIN_hidePick" + 10 * i + j);
		}
		document
			.getElementById("FIN_Pack" + 10 * i)
			.classList.remove("FIN_hidePack" + 10 * i);
	}
	...
	for (let i = 1; i <= 3; i++) {
		for (let j = 20; j <= 30; j += 10) {
			...
			document
				.getElementById("FIN_" + i + "." + j)
				.classList.remove("FIN_pickedQuestion");
		}
	}
	...
	for (let i = 2; i <= 3; i++) {
		document
			.getElementById("FIN_Pack" + 10 * i)
			.classList.remove("FIN_movePack" + 10 * i);
		var pack = document.getElementById("FIN_Pack" + 10 * i);
		void pack.offsetWidth;
		document
			.getElementById("FIN_Pack" + 10 * i)
			.classList.add("FIN_movePack" + 10 * i);

		for (let j = 1; j <= 3; j++) {
			var tempId = "FIN_" + j + "." + 10 * i;
			document
				.getElementById(tempId)
				.classList.remove("FIN_showPick" + 10 * i + j);
			var pick = document.getElementById(tempId);
			void pick.offsetWidth;
			document
				.getElementById(tempId)
				.classList.add("FIN_showPick" + 10 * i + j);
		}
	}
});
...
function FIN_printPlayerData() {
	for (let i = 0; i < 4; i++) {
		...
		if (i + 1 != Number(FIN_currentPlayer))
			document.getElementById("FIN_Data" + (i + 1)).textContent =
				playerName + " (" + playerPoint + ")";
		else document.getElementById("FIN_Data" + (i + 1)).textContent = playerName;
	}
	if (FIN_currentPlayer)
		document.getElementById("FIN_currentPoint").innerHTML =
			allPlayerPoint[Number(FIN_currentPlayer) - 1];
}
...
socket.on("_FIN_packChosen", function (list) {
	...
	for (let i = 1; i <= 3; i++) {
		...
		for (let j = 1; j <= 3; j++) {
			...
			if (i != 1) {
				...
				document
					.getElementById(tempId)
					.classList.add("FIN_hidePick" + 10 * i + j);
				document
					.getElementById("FIN_Pack" + 10 * i)
					.classList.remove("FIN_hidePack" + 10 * i);
				var pack = document.getElementById("FIN_Pack" + 10 * i);
				...
				document
					.getElementById("FIN_Pack" + 10 * i)
					.classList.add("FIN_hidePack" + 10 * i);
			}
		}
	}
	for (let i = 1; i <= 3; i++) {
	...
	document
		.getElementById("FIN_Shelf")
		.classList.remove("FIN_moveQuestionShelf");
	var shelf = document.getElementById("FIN_Shelf");
	void shelf.offsetWidth;
	document.getElementById("FIN_Shelf").classList.add("FIN_moveQuestionShelf");

	document
		.getElementById("FIN_Point" + FIN_currentPlayer)
		.classList.remove("FIN_differentPoint");
	document
		.getElementById("FIN_questionBox")
		.classList.remove("FIN_questionBoxMove");
	document
		.getElementById("FIN_questionStatus")
		.classList.remove("FIN_movePointAndStatus");
	document
		.getElementById("FIN_currentPoint")
		.classList.remove("FIN_movePointAndStatus");
	for (let i = 1; i <= 4; i++) {
	...
	document
		.getElementById("FIN_questionBox")
		.classList.add("FIN_questionBoxMove");
	document
		.getElementById("FIN_questionStatus")
		.classList.add("FIN_movePointAndStatus");
	document
		.getElementById("FIN_currentPoint")
		.classList.add("FIN_movePointAndStatus");
	for (let i = 1; i <= 4; i++) {
		document
			.getElementById("FIN_Point" + i)
			.classList.add("FIN_showPointBar" + i);
	}
	setTimeout(function () {
		...
		document
			.getElementById("FIN_Point" + FIN_currentPlayer)
			.classList.add("FIN_differentPoint");
		FIN_printPlayerData();
	}, 4800);
});
...
socket.on("_FIN_chooseQuestion", function (question) {
	for (let i = 1; i <= 3; i++) {
		if (i == Number(question.ithQuestion))
			document.getElementById("FIN_" + i).classList.add("FIN_chosenQuestion");
		else
			document
				.getElementById("FIN_" + i)
				.classList.remove("FIN_chosenQuestion");
	}
	document.getElementById("FIN_Question").textContent =
...
socket.on("_FIN_startTiming", function (questionTime) {
	for (let i = 10; i <= 20; i += 5) {
		document
			.getElementById("FIN_Circle")
			.classList.remove("FIN_moveCircle" + i);
	...
	document
		.getElementById("FIN_Circle")
		.classList.add("FIN_moveCircle" + questionTime);
	document
		.getElementById("FIN_Line")
		.classList.add("FIN_moveLine" + questionTime);
});
...
socket.on("_SFI_startRound", function (playerList) {
	...
	for (let i = 0; i < playerList.length; i++) {
		dadEle.innerHTML +=
			...
			playerList[i] +
			"></label></span>";
		document.getElementById("SFI_" + playerList[i]).style.visibility =
			"visible";
	}
	SFI_printPlayerData();
});
...
socket.on("_SFI_openQuestion", function (serverData) {
	...
	document.getElementById("SFI_Question").textContent =
		serverData.questionData.question;
	document.getElementById("SFI_roundName").textContent =
		"CÂU HỎI PHỤ " + serverData.openedCount + "/3";
	SFI_resetStatus();
});
