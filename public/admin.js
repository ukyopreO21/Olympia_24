...
function scrollDown() {
	...
	let isAtBottom =
		chatbox.scrollHeight - chatbox.scrollTop <= chatbox.clientHeight * 1.1;
	return isAtBottom;
}
...
socket.on("serverData", function (data) {
	...
	for (let i = 1; i <= 4; i++) {
		document.getElementById("player" + i + "Name").value =
			data.playerName[i - 1];
		document.getElementById("player" + i + "Point").value =
			data.playerPoint[i - 1];
		if (data.isReady[i - 1])
			document.getElementById("TS" + i).style.color = "yellowgreen";
	}
});

socket.on("_sendReady", function (data) {
	if (data.ready)
		document.getElementById("TS" + data.playerNumber).style.color =
			"yellowgreen";
...
function Select() {
	...
	if (roundID == "1") {
		...
		temp.innerHTML +=
			'<span class="button" onclick="playIntro()">Intro</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="startRound()">Bắt đầu</span><br>';
		temp.innerHTML += "<span class='text'>Lượt:&nbsp</span>";
		temp.innerHTML += '<input id="STR_ithStart"></input> ';
		temp.innerHTML +=
			'<span class="button" onclick="STR_choosePlayer()">Confirm</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="STR_startPlayerTurn()">Vào lượt</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="STR_openQuestionBoard()">Mở bảng câu hỏi</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="STR_startTurn()">Bắt đầu lượt thi</span><br>';
		...
		temp.innerHTML +=
			'<span class="button" onclick="STR_5s()">5s trả lời</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="STR_3s()">3s khoá chuông</span><br>';
		...
		temp.innerHTML +=
			'<span class="button" onclick="STR_getNextQuestion()">Chuyển câu</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="STR_finishTurn()">Hoàn thành lượt</span>';
		openEndPart();
	} else if (roundID == "2") {
		...
		temp.innerHTML +=
			'<span class="button" onclick="startRound()">Bắt đầu</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_showNumberOfCharacter()">Tiết lộ số kí tự</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_showRows()">Hiện hàng ngang</span><br>';
		temp.innerHTML += "<span class='text'>Chọn hàng ngang số:&nbsp</span>";
		temp.innerHTML += '<input id="OBS_ithRow"></input> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_chooseRow()">Confirm</span><br>';
		temp.innerHTML += "<span class='text'>Khu vực câu hỏi:&nbsp</span>";
		temp.innerHTML +=
			'<span class="button" onclick="OBS_showRowQuestion()">Mở câu hỏi</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_closeRowQuestion()">Tắt câu hỏi</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_start15s()">15 giây</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_last15s()">15 giây cuối</span><br>';
		temp.innerHTML += "<span class='text'>Thời gian:&nbsp</span>";
		temp.innerHTML += '<span class="text" id="OBS_rowObsTime">0</span><br>';
		temp.innerHTML += "<span class='text'>Khu vực hàng ngang:&nbsp</span>";
		temp.innerHTML +=
			'<span class="button" onclick="OBS_showRowAnswer()">Hiện đáp án hàng ngang</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_rightRow()">Đúng hàng ngang</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_wrongRow()">Sai hàng ngang</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_backScreen()">Về giao diện chính</span><br>';
		temp.innerHTML += "<span class='text'>Khu vực CNV:&nbsp</span>";
		temp.innerHTML +=
			'<span class="button" onclick="OBS_rightObs()">Đúng CNV</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_wrongObs()">Sai CNV</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_showObstacle()">Mở CNV (không ai đúng)</span><br>';
		temp.innerHTML += "<span class='text'>Khu vực ảnh:&nbsp</span>";
		temp.innerHTML +=
			'<span class="button" onclick="OBS_ImageUI()">Giao diện ảnh</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_openCorner()">Mở góc ảnh</span><br>';
		temp.innerHTML += "<span class='text'>Số điểm tối đa:&nbsp</span>";
		temp.innerHTML +=
			'<b><span class="text" id="OBS_maxObsPoint">60</span></b><br>';
		temp.innerHTML += "<span class='text'>Điểm khuyến nghị CNV:&nbsp</span>";
		temp.innerHTML +=
			'<b><span class="text" id="OBS_suggestObsPoint">60</span></b> ';
		temp.innerHTML += '<span class="button" onclick="OBS_addPoint()">+</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_minusPoint()"> - </span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_deleteSignal()">Xoá tín hiệu</span>';
		openEndPart();
	} else if (roundID == "3") {
		temp.innerHTML += "<div class='inRound'>TRONG PHẦN THI</div>";
		temp.innerHTML +=
		...
		temp.innerHTML +=
			'<span class="button" onclick="ACC_chooseQuestion()">Confirm</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_openQuestion()">Mở câu hỏi</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_startTiming()">Bắt đầu tính giờ</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_turnOffQuestion()">Tắt câu hỏi</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_clearData()">Xoá dữ liệu</span><br>';
		temp.innerHTML += "<span class='text'>Thời gian:&nbsp</span>";
		temp.innerHTML += '<span class="text" id="ACC_Time">0</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_showAnswers()">Hiện đáp án thí sinh</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_showQuestionAnswer()">Hình ảnh đáp án</span><br>';
		temp.innerHTML += '<span class="button" onclick="ACC_Right()">Đúng</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_Wrong()">Sai</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_questionScreen()">Giao diện câu hỏi</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="ACC_answerScreen()">Giao diện đáp án</span>';
		openEndPart();
	} else if (roundID == "4") {
		temp.innerHTML += "<div class='inRound'>TRONG PHẦN THI</div>";
		temp.innerHTML +=
		...
		temp.innerHTML +=
			'<span class="button" onclick="FIN_choosePlayer()">Confirm</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_showQuestionPack()">Chọn gói</span><br>';
		temp.innerHTML += "<span class='text'>Câu hỏi số:</span> ";
		temp.innerHTML += '<input id="FIN_ithQuestion"> ';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_chooseQuestion()">Mở câu hỏi</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_Star()">Ngôi sao hi vọng</span> ';
		temp.innerHTML +=
			'<span class="text" id="FIN_starStatus">Trạng thái: Đang tắt</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_startTiming()">Bắt đầu tính giờ</span> ';
		temp.innerHTML += "<span class='text'>Thời gian:&nbsp</span>";
		temp.innerHTML += '<span class="text" id="FIN_Time">0</span><br>';
		temp.innerHTML += '<span class="button" onclick="FIN_Right()">Đúng</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_5s()">5 giây giành quyền</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_Wrong()">Sai</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_deleteSignal()">Xoá tín hiệu</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="FIN_finishTurn()">Hoàn thành lượt</span>';
		openEndPart();
	} else if (roundID == "5") {
		temp.innerHTML += "<div class='inRound'>TRONG PHẦN THI</div>";
		temp.innerHTML +=
			'<span class="button" onclick="SFI_choosePlayers()">Chọn thí sinh</span><br>';
		temp.innerHTML += '<div id="SFI_Players"></div>';
		temp.innerHTML += "<span class='text'>Câu hỏi số</span> ";
		temp.innerHTML += '<input id="SFI_ithQuestion"> ';
		temp.innerHTML +=
			'<span class="button" onclick="SFI_openQuestion()">Mở câu hỏi</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="SFI_closeQuestion()">Tắt câu hỏi</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="SFI_startTiming()">Bắt đầu tính giờ</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="SFI_updateScore()">Cập nhật điểm</span><br>';
		temp.innerHTML += "<span class='text'>Thời gian:&nbsp</span>";
		temp.innerHTML += '<span class="text" id="SFI_Time">0</span><br>';
		temp.innerHTML += '<span class="button" onclick="SFI_Right()">Đúng</span> ';
		temp.innerHTML +=
			'<span class="button" onclick="SFI_Continue()">Đồng hồ</span> ';
		temp.innerHTML += "<span class='text'>Trạng thái:</span> ";
		temp.innerHTML +=
			'<span class="text" id="SFI_clockStatus">Đang dừng</span><br>';
		temp.innerHTML +=
			'<span class="button" onclick="OBS_deleteSignal()">Xoá tín hiệu</span>';
		openEndPart();
	}
...
socket.on("currentUI", function () {
	let roundID = document.getElementById("rounds").value;
	let UIName = document.getElementById("UIName").textContent;
	socket.emit("_currentUI", { isChatBan, roundID, UIName });
});
...
function OBS_addPoint() {
	let currentObsPoint = Number(
		document.getElementById("OBS_suggestObsPoint").textContent
	);
...
function Confirm() {
	...
	for (let i = 1; i <= 4; i++) {
		currentPlayerPoint[i - 1] = Number(
			document.getElementById("player" + i + "Point").value
		);
		currentPlayerName[i - 1] = document.getElementById(
			"player" + i + "Name"
		).value;
	}
...
socket.on("_OBS_adminGetRoundData", function (data) {
	...
	temp +=
		"<div>Chướng ngại vật có " +
		OBS_Length +
		" kí tự: <font color='cyan'><b>" +
		data.OBS_CNV.answer +
		"</b></font></div>";
...
function OBS_chooseRow() {
	...
	document.getElementById("OBS_suggestObsPoint").textContent =
		60 - (count - 1) * 10 - OBS_pointChange;

	if (rowIth == 5)
		document.getElementById("OBS_suggestObsPoint").textContent = 10;
	for (i = 1; i <= 4; i++) {
...
socket.on("_OBS_playerObsSignal", function (signalData) {
	document.getElementById("signal" + signalData.playerNumber).innerHTML =
		signalData.OBS_numberOfObsSignal;
	document.getElementById("p" + signalData.playerNumber).style.border =
		"calc(5vw/48) solid #73f7ff";
	let name = document.getElementById(
		"name" + signalData.playerNumber
	).textContent;
...
socket.on("_STR_getNextQuestion", function (questionData) {
	...
	if (STR_signalPlayer)
		document.getElementById("p" + STR_signalPlayer).style.border =
			"calc(5vw/96) dotted grey";
...
socket.on("_STR_blockSignal", function (playerNumber) {
	...
	document.getElementById("p" + playerNumber).style.border =
		"calc(5vw/48) solid #73f7ff";
});
...
function STR_Right() {
	...
	if (ithStart != 5) {
		...
		document.getElementById("player" + ithStart + "Point").value =
			Number(point) + 10;
	} else {
		let point = document.getElementById(
			"player" + STR_signalPlayer + "Point"
		).value;
		document.getElementById("player" + STR_signalPlayer + "Point").value =
			Number(point) + 10;
	}
...
socket.on("_ACC_sentAnswer", function (serverData) {
	...
	document.getElementById(answerLabel).innerHTML =
		serverData.answerData.playerAnswer;
...
function ACC_showAnswers() {
	...
	for (let i = 0; i < 4; i++) {
		answerData[i] = {};
		answerData[i].answer = document.getElementById(
			"answer" + (i + 1)
		).textContent;
		answerData[i].name = document.getElementById("name" + (i + 1)).textContent;
		answerData[i].time = Number(
	...
	for (i = 0; i < 4; i++) {
		...
		for (j = 0; j < 4; j++) {
			if (
				document.getElementById("time" + (j + 1)).textContent ==
					answerData[i].time &&
				answerData[i].answer != ""
			) {
	...
	for (i = 0; i < 4; i++) {
		...
		point = Number(
			document.getElementById(
				"player" + ACC_playerAnswerInfo[i].playerNumber + "Point"
			).value
		);
		document.getElementById(
			"player" + ACC_playerAnswerInfo[i].playerNumber + "Point"
		).value = point + ACC_playerAnswerInfo[i].point;
	}
	Confirm();
}
...
function ACC_Right() {
	...
	for (i = 1; i <= 4; i++) {
		...
		ACC_playerAnswerInfo[i - 1].time = Number(
			document.getElementById("time" + i).textContent
		);
		ACC_playerAnswerInfo[i - 1].playerNumber = i;
		ACC_playerAnswerInfo[i - 1].checked = document.getElementById(
			"check" + i
		).checked;
		ACC_playerAnswerInfo[i - 1].point = 0;
		ACC_playerAnswerInfo[i - 1].signal = document.getElementById(
			"signal" + i
		).textContent;
	}
...
function FIN_showQuestionPack() {
	...
	temp += "<tr>";
	temp +=
		'<th id="blank"><button id="FIN_packChosen" onclick="FIN_packChosen()">CHỐT GÓI</button></th>';
	temp += "<th>Câu 1</th>";
	temp += "<th>Câu 2</th>";
	temp += "<th>Câu 3</th>";
	temp += "</tr>";
	...
	temp +=
		'<td><input type="radio" name="1" value="20" onclick="FIN_Choose(this)"></td>';
	temp +=
		'<td><input type="radio" name="2" value="20" onclick="FIN_Choose(this)"></td>';
	temp +=
		'<td><input type="radio" name="3" value="20" onclick="FIN_Choose(this)"></td>';
	temp += "</tr>";
	temp += "<tr>";
	temp += '<td style="color: #f1a621">30 điểm</td>';
	temp +=
		'<td><input type="radio" name="1" value="30" onclick="FIN_Choose(this)"></td>';
	temp +=
		'<td><input type="radio" name="2" value="30" onclick="FIN_Choose(this)"></td>';
	temp +=
		'<td><input type="radio" name="3" value="30" onclick="FIN_Choose(this)"></td>';
	temp += "</tr>";
	temp += "</table>";
	document.getElementById("FIN_chooseQuestionBoard").innerHTML += temp;
	document.getElementById("FIN_chooseQuestionBoard").style.visibility =
		"visible";
...
socket.on("_FIN_chooseQuestion", function (question) {
	...
	if (FIN_currentQuestionPoint == 10)
		document.getElementById("FIN_Time").textContent = 10;
	else if (FIN_currentQuestionPoint == 20)
		document.getElementById("FIN_Time").textContent = 15;
...
function FIN_Star() {
	...
	if (FIN_isStarOn) {
		FIN_isStarOn = false;
		document.getElementById("FIN_starStatus").textContent =
			"Trạng thái: Đang tắt";
	} else {
		FIN_isStarOn = true;
		document.getElementById("FIN_starStatus").textContent =
...
function FIN_Right() {
	if (FIN_signalPlayer) {
		...
		point = Number(
			document.getElementById("player" + FIN_signalPlayer + "Point").value
		);
		...
		if (!FIN_isStarOn) {
			...
			point = Number(
				document.getElementById("player" + FIN_currentPlayer + "Point").value
			);
			point -= FIN_currentQuestionPoint;
			if (point < 0) point = 0;
			document.getElementById("player" + FIN_currentPlayer + "Point").value =
...
function FIN_5s() {
	if (FIN_isStarOn) {
		...
		let point = Number(
			document.getElementById("player" + currentPlayer + "Point").value
		);
		point -= FIN_currentQuestionPoint;
		if (point < 0) point = 0;
...
function SFI_choosePlayers() {
	...
	if (document.getElementById("SFI_startRound"))
		document.getElementById("SFI_startRound").style.display = "block";
	temp = document.getElementById("SFI_Players");
	temp.innerHTML = "";
	for (i = 1; i <= 4; i++) {
		temp.innerHTML +=
			'<span class="button" data-ispicked="false" id="SFI_' +
			i +
			'" onclick="SFI_Pick(this)"></span> ';
		document.getElementById("SFI_" + i).textContent = document.getElementById(
			"player" + i + "Name"
		).value;
	}
	temp.innerHTML +=
		'<span class="button" id="SFI_startRound" onclick="SFI_startRound()">Bắt đầu vòng thi</span>';
	temp.innerHTML += "<br>";
}

function SFI_Pick(buttonData) {
	if (buttonData.getAttribute("data-ispicked") == "false") {
		document.getElementById(buttonData.id).style.border =
			"calc(5vw/48) solid #df2453";
		document.getElementById(buttonData.id).dataset.ispicked = "true";
	} else {
		document.getElementById(buttonData.id).style.border =
			"calc(5vw/48) solid #f1a621";
...
function SFI_startRound() {
	...
	for (i = 1; i <= 4; i++) {
		if (
			document.getElementById("SFI_" + i).getAttribute("data-ispicked") ==
			"true"
		) {
...
function SFI_Timing(startTime, leftTime) {
	...
	downloadTimer = setInterval(function () {
		...
		document.getElementById("SFI_Time").textContent = (
			leftTime -
			(Number(currentTime) - Number(startTime))
		).toFixed(2);
